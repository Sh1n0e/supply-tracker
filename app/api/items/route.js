...
export async function GET(request) {
  ...
  const items = locationId
    ? await sql`
        SELECT
          i.item_id, i.name, i.quantity, i.unit, i.par_level,
...
export async function POST(request) {
  ...
  const [item] = await sql`
    INSERT INTO items (name, quantity, unit, par_level, location_id, category_id, purchase_date)
    VALUES (${name}, ${quantity}, ${unit || "item"}, ${par_level ?? null}, ${location_id}, ${category_id ?? null}, CURRENT_DATE)
    RETURNING *
  `;
